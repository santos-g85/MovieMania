...
# Global variables for your data
movies = None
similarity = None
...
def get_recommendations(request):
    ...
    # Loop through recommendations and fetch details for each
    for movie in recommendations:
        details_response = fetch_movie_details(movie['movie_id'])  # Call the fetch function
        
        if details_response.status_code == 200:  # Check if the request was successful
            movie_details.append(details_response.data)  # Append the movie details
...
def fetch_movie_details(movie_id):
    ...
    try:
        ...
        if response.status_code == 200:
            return Response(response.json())  # Return the data as JsonResponse
        else:
